...
### ensure all ssh trafic is not tored
iptables -t nat -A PREROUTING -i $_inc_if -p tcp --dport 22 -j ACCEPT
iptables -t nat -A PREROUTING -i $_out_if -p tcp --dport 22 -j ACCEPT
iptables -A INPUT -p tcp --dport 22 -j ACCEPT
iptables -A OUTPUT -p tcp --dport 22 -j ACCEPT


### *nat PREROUTING (For middlebox)
sudo iptables -t nat -A PREROUTING -d $_virt_addr -i $_inc_if -p tcp -m tcp --tcp-flags FIN,SYN,RST,ACK SYN -j REDIRECT --to-ports $_trans_port
...
sudo iptables -A INPUT -i $_out_if -p tcp --dport 22 -m state --state NEW -j ACCEPT
...
sudo iptables -A OUTPUT -o $_out_if -m owner --uid-owner $_stubby_uid -p tcp -m tcp --tcp-flags FIN,SYN,RST,ACK SYN -m state --state NEW -j ACCEPT
